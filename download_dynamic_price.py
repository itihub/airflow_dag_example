...
def save_to_mysql_stage(*args, **context):
    tickers = context["dag_run"].conf.get("stocks")
...
def generate_dag(investment_type):
    with DAG(
        dag_id = f'downlaod_{investment_type}_price',
        default_args = default_args,
        description = f'This Dag downloads {investment_type} price and save them into text files',
        ...
        download_task = PythonOperator(
            task_id = f"download_{investment_type}_price",
...
investment_type = ['Equity', 'ETF']
for _type in investment_type:
    globals()["Dynamic_DAG" + _type] = generate_dag(_type) # 将新生成的DAG放在全局变量中

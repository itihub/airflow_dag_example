...
select ticker, as_of_date, open_price, high_price, low_price, close_price from stock_prices_stage s 
where not exists
    (select 1 from stock_prices p where p.ticker = s.ticker and p.as_of_date = s.as_of_date);
